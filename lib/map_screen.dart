...
class _FixedMapScreenState extends State<FixedMapScreen>
  ...
  void initState() {
    ...
    _subscribeToStudentsStream(); // Subscribe to the new stream
  }

  /// Subscribes to the stream of all students from MapService.
  void _subscribeToStudentsStream() {
    _studentsStreamSubscription = _mapService.getStudentsStream().listen((allStudents) {
      if (mounted) {
        setState(() {
          ...
          // Filter out the current user to avoid displaying them twice on the map
          _otherStudents =
              allStudents.where((s) => s.id != _student.id).toList();
        });
      }
    });
  }
  ...
  Widget _buildMap() {
    List<Marker> allMarkers = [
      ...
      // Markers for all other students
      ..._otherStudents.map((student) {
        return Marker(
          width: 60,
          height: 60,
